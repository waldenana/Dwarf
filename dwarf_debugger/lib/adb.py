...
class Adb(QObject):
    ...
    def _check_is_su(self):
        res = self._do_adb_command('su')
        self._is_su = 'not found' not in res

    def disable_enforce(self):
    ...
    def _check_requirements(self):  # pylint: disable=too-many-branches, too-many-statements
        ...
        if not self._device_serial:
            return
        self._dev_emu = self._check_device()
        self._check_is_root()
        self._check_is_su()
    ...
    def available(self):
        ...
        """
        if self._is_root or self._is_su:
            return True
    ...
    def start_frida(self, daemonize=True, restart=False):
        ...
        if not daemonize:
            if self._alternate_frida_name:
                result = self.su_cmd('/data/local/tmp/frida-server &')
            else:
                result = self.su_cmd('/data/local/tmp/frida &')
        else:
            # with nox it starts frida fine but keeps running
            # without return so it needs some timeout here
            if self._alternate_frida_name:
                result = self.su_cmd('/data/local/tmp/frida-server -D', timeout=5)
            else:
                result = self.su_cmd('/data/local/tmp/frida -D', timeout=5)
    ...
    def get_frida_version(self):
        ...
        result = self._do_adb_command('shell  /data/local/tmp/frida-server --version')
        if result:
    ...
    #         self._system_name = '/sbin/.magisk/mirror/system'
    #     if '/system_root' in res:
    #         self._system_name = '/system_root'
    #         if '/sbin/.magisk/block/system_root /' in res:
    #             self._system_name = '/sbin/.magisk/mirror/system_root'
    #     res = self.su_cmd('mount -o rw,remount ' + self._system_name)
    #
    #     if res == '':
